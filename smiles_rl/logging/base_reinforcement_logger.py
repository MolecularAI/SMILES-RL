...
class BaseReinforcementLogger(ABC):
    ...
    def save_checkpoint(self, step, scaffold_filter, agent):
        actual_step = step + 1
        if (
            self._log_config.logging_frequency > 0
            and actual_step % self._log_config.logging_frequency == 0
        ):
            if scaffold_filter is not None:
                self.save_filter_memory(scaffold_filter)
            agent.save_to_file(
                os.path.join(
                    self._log_config.result_folder, f"Agent.{actual_step}.ckpt"
                )
            )
    ...
    def save_filter_memory(self, scaffold_filter):
        ...
        self.save_to_csv(
            scaffold_memory, self._log_config.result_folder, self._log_config.job_name
        )
    ...
    def save_to_csv(self, scaffold_memory, path, job_name="default_job"):
        ...
        if len(scaffold_memory) > 0:
            sorted_df = scaffold_memory.sort_values(
                sf_enum.TOTAL_SCORE, ascending=False
            )
            sorted_df["ID"] = [
                f"{job_name}_{e}" for e, _ in enumerate(sorted_df.index.array)
            ]
