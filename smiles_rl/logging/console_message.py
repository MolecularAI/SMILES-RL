...
class ConsoleMessage:
    def create(
        ...
        time_message = self._time_progress(
            start_time, n_steps, step, smiles, mean_score, loss
        )
        score_message = self._score_profile(
    ...
    def _time_progress(self, start_time, n_steps, step, smiles, mean_score, loss):
        ...
        message = (
            ...
            f"Augmented loss: {mean_score:.8f}\n"
        )
        return message

    def _score_profile(
        self, smiles, agent_likelihood, prior_likelihood, augmented_likelihood, score
    ):
        ...
        for i in range(min(10, len(smiles))):
            message += (
                f"{agent_likelihood[i]:6.2f}    {prior_likelihood[i]:6.2f}    "
