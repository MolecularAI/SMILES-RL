...
from .configuration import _is_development_environment
from ..utils.general import estimate_run_time
...
from .remote_logging import get_remote_logging_auth_token
...
class RemoteReinforcementLogger(BaseReinforcementLogger):
    ...
    def timestep_report(
        ...
        score_components = self._score_summary_breakdown(
            score_summary, mean_score, diversity_filter
        )
        learning_curves = self._learning_curve_profile(
        ...
        time_estimation = estimate_run_time(start_time, n_steps, step)
        data = self._assemble_timestep_report(
            step,
            score_components,
            structures_table,
            learning_curves,
            time_estimation,
            ul_rl.fraction_valid_smiles(smiles),
            smiles_report,
        )
    ...
    def _visualize_structures(self, smiles, score, score_summary: FinalSummary):
        ...
        mol_in_base64_string = ul_rl.mol_to_png_string(
            list_of_mols,
            molsPerRow=self._columns,
            subImgSize=(300, 300),
            legend=legend,
            matches=pattern,
        )
    ...
    def _create_sample_report(self, smiles, score, score_summary: FinalSummary):
        score, smiles = ul_rl.sort_smiles_by_score(score, smiles)
        smiles = ul_rl.padding_with_invalid_smiles(smiles, self._sample_size)
        _, legend = ul_rl.check_for_invalid_mols_and_create_legend(
            smiles, score, self._sample_size
        )
        smarts_pattern = self._get_matching_substructure_from_config(score_summary)

        smiles_legend_pairs = [
            {"smiles": smiles[indx], "legend": legend[indx]}
            for indx in range(self._sample_size)
        ]
    ...
    def _learning_curve_profile(
        self, agent_likelihood, prior_likelihood, augmented_likelihood
    ):
        learning_curves = {
