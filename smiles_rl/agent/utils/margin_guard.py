...
class MarginGuard:
    ...
    def adjust_margin(self, step: int) -> None:
        if step == self.margin_window:
            if len(self._run_stats) < self.margin_window:
                raise Exception(
                    f"self._run_stats has {len(self._run_stats)} elements. Consider storing all stats!"
                )
    ...
    @torch.no_grad()
    def get_distance_to_prior(
        self,
        prior_likelihood: Union[torch.Tensor, np.ndarray],
        ...
        if type(prior_likelihood) == torch.Tensor:
            ...
            mask = torch.where(
                prior_likelihood > distance_threshold,
                ones,
                distance_threshold / prior_likelihood,
            )
            mask = mask.cpu().numpy()
        else:
