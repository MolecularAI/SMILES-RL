...
class RegularizedMLENovelty(BaseAgent):
    def __init__(
        self,
    ...
    def update(self, smiles: List[str]):
        ...
        # Adds intrinsic reward and/or penalty on extrinsic reward
        ...
        # switch signs
        agent_likelihood = -self._actor.likelihood_smiles(sample_smiles)
    ...
    def act(self, batch_size: int) -> List[str]:
        ...
        seqs, smiles, agent_likelihood = sample_unique_sequences(
    ...
    def _stats_and_chekpoint(
        ...
        timestep_report = (
            f"\n Reg. MLE Step {step}   Fraction valid SMILES: {valid_fraction:4.1f}   Score: {mean_score:.4f}   "
    ...
    def reset(self, reset_countdown: int = 0) -> int:
        ...
        print("Resetting Regularized MLE model", flush=True)

        self._actor = DefaultModel.load_from_file(
        ...
        assert (
            self._prior.get_vocabulary() == self._actor.get_vocabulary()
